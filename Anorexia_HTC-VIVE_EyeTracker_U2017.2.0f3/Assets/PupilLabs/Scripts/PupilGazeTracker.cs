...
public class PupilGazeTracker:MonoBehaviour
{
	...
	void NetMQClient()
	{
        ...
        if (_isconnected) {
            Debug.Log("Connected to the server: " + IPHeader + ServicePort + ".");
            UnityMainThreadDispatcher.Instance().Enqueue(pupilConnection());
            StartProcess ();
            ...
            UnityMainThreadDispatcher.Instance().Enqueue(pupilTerminated());
            StopProcess ();
			subscriberSocket.Close();
		}
		else
        {
            string text = "Failed to connect the server.";
            Debug.Log(text);
            UnityMainThreadDispatcher.Instance().Enqueue(pupilDisconnection());
        }
    ...
    public IEnumerator pupilDisconnection()
    {
        this.anorexiaUB.pupilDisconnection();
        yield return null;
    }

    public IEnumerator pupilConnection()
    {
        this.anorexiaUB.pupilConnection();
        yield return null;
    }

    public IEnumerator pupilTerminated()
    {
        this.anorexiaUB.pupilTerminated();
        yield return null;
    }

    public IEnumerator calibrationTerminated() {
		...
		if (m_status == EStatus.ProcessingGaze) { //gaze processing stage
			...
			//	pointsData = JsonUtility.ToJson (_CalibrationPoints);
				...
				if (_currCalibPoint >= _calibPoints.Length) {
					...
					StopCalibration ();
                    UnityMainThreadDispatcher.Instance().Enqueue(this.calibrationTerminated());
                }
